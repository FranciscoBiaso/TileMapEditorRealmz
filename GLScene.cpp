...
GLRect& GLScene::getQuad(int index)
{
    return _quads[index];
}

void GLScene::setScaleFactor(float scale)
{
    scaleFactor = scale;
}

gboolean GLScene::static_render(GtkGLArea* area, GdkGLContext* context, gpointer user_data)
{
    return reinterpret_cast<GLScene*>(user_data)->render(area, context);
}

gboolean GLScene::static_realize(GtkGLArea* area, gpointer user_data)
{
    return reinterpret_cast<GLScene*>(user_data)->realize(area);
}

void GLScene::static_resize(GtkGLArea* area, gint width, gint height, gpointer user_data)
{
    reinterpret_cast<GLScene*>(user_data)->resize(area, width, height);
}

int GLScene::getWidth(){ return width;}
int GLScene::getHeigth(){return height;}

GtkWidget* GLScene::getGLArea()
{
    return _gtkGLArea;
}

gboolean GLScene::render(GtkGLArea* area, GdkGLContext* context)
{
...
gboolean GLScene::realize(GtkGLArea* area)
{
    // ADD DEPTH BUFFER //
...
void GLScene::setCamera(glm::vec2 center, float z)
{
    glm::mat4 zoom(1.0f);
...
glm::vec3 GLScene::getCameraCenter()
{
...
void GLScene::disableQuadShadow()
{
    quad_shadow_on = false;
}

void GLScene::enableQuadShadow()
{
    quad_shadow_on = true;
}

void GLScene::disableQuadSelection()
{
    quad_selection_on = false;
}

void GLScene::enableQuadSelection()
{
    quad_selection_on = true;
}

void GLScene::get_LeftTop_rightbot(glm::vec2 A, glm::vec2 B, glm::vec2 & leftTop, glm::vec2 & rightBot)
{
    if (A.y > B.y && A.x < B.x)
    {
        leftTop = A;
...
void GLScene::updateSelectionQuad(glm::vec2 A, glm::vec2 B, glm::vec4 colorRect, glm::vec4 colorBorder, float zCoord)
{
    glm::vec2 leftTop;
    glm::vec2 rightBot;
...
void GLScene::removeLightCylindergMapUI(int index)
{
